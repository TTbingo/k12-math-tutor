...
[![Version](https://img.shields.io/badge/version-2.0-2563eb?style=flat-square)](SKILL.md)
[![License](https://img.shields.io/badge/license-MIT-22c55e?style=flat-square)](LICENSE)
[![Tests](https://img.shields.io/badge/tests-67%20passing-16a34a?style=flat-square)](scripts/tests/)
...
- **19 条硬约束**：if-then 规则，覆盖算理、追问、情绪、双输出、原题呈现、家长友好降级等
- **13 项自检清单**：输出前逐项打勾，4 项强制阻塞
- **pytest 回归套件**：67 用例全绿，覆盖全部 5 个验证工具
...
└── references/                                # 知识资产（36 个文件，扁平结构）
    ...
    ├── constraints-quick-ref.md              # 19 条硬约束速查卡
    ├── core-formula-teaching-scripts.md
    ├── curriculum-standard-2022.md
    ├── delivery-workflow.md                   # 双发 / 可视化图表交付工作流（B1 抽出）
    ...
    ├── student-mode-playbook.md              # 学生模式 prose（B1 抽出）
    ├── tutor-log.md
    ├── xuanba-problem-bank.md
    ├── _TEMPLATE.md                          # 新增参考文件模板
    └── archive/                              # 历史归档（非活跃）
```
...
# 答案可逆验证（方程模式：解出的值代回原方程校验）
python scripts/verify-answer.py --equation "x-3=5" --answer "x=8"

# 答案可逆验证（不等式模式）
python scripts/verify-answer.py --check "a>b" --option "b-a>0"

# 方法论偏离检测
python scripts/diff-case.py output_v1.md output_v2.md

# Skill 结构完整性校验
python scripts/validate-structure.py --dir .

# 算理考点识别（文本模式）
python scripts/extract-equation.py --text "某超市购进大米，成本4元/kg..."

# 算理考点识别（图片模式：OCR 尚未实现，仅做路径校验与互斥提示）
python scripts/extract-equation.py --image ./exam.png
```
...
<p>
内置答案可逆验证（将答案代回原题验证），加 67 个 pytest 回归用例。但如果发现错误，请提交 issue 触发飞轮修复——这是 Skill 越用越准的设计机制。
</p>
</details>
