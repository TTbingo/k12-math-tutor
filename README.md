...
[![Version](https://img.shields.io/badge/version-2.17.0-blue)](SKILL.md)
...
| **课标核心素养** | 2022 课标 + 2017 高教版 | 三会用 + 十大核心素养 + 跨学段衔接 |
...
| 初中 | `junior-math-review-compendium.md` | 26 章中考总复习（718 行） |
| 高中 | `hs-math-review-compendium.md` | 五册 18 章深度萃取（3085 行） |

### 飞轮进化机制

每次真实辅导中遇到失败模式 → 自动追加到 `gotchas.md`（G1-G29 + A1-A5）→ 升级约束规则 → Skill 越用越准。不是静态文档，是活的经验库。
...
├── README.md                              # 本文件
├── test-prompts.json                      # 测试用例集（39 题）
├── evals/
│   └── eval-set.md                        # 20 题活体测试（4 维打分）
├── scripts/
...
└── references/                            # 27+ 个参考资产
    ...
    ├── hs-math-curriculum-2017.md         # 高中课标 2017 版
    ├── hs-math-review-compendium.md       # 高中五册深度萃取（18 章）
...
**v2.13.0** — Evals + 自动化：20 题活体测试 + 3 个 Python 脚本 + 月度评估模板。

**v2.12.0** — 方法论快速调用矩阵 + 微信双发 SOP + 案例精选瘦身（14→5）。

**v2.11.0** — 架构重构：SKILL.md 从 437 行瘦身到 218 行，拆分到 references/，新增飞轮追加机制。
